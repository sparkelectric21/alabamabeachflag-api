...
export function parseOperationalControl(value: unknown): OperationalControlDocument | null {
	...
	const legacyRequired = CONTROL_IDS.filter((id) => !["providers.gulfStateParkEvents", "providers.orangeBeachCoastalEvents", "notifications.beachActivity", "domains.vibrioAwareness"].includes(id));
...
export function evaluateVibrioAwarenessControl(doc: OperationalControlDocument, now = new Date()): EffectiveControl {
	return evaluateControls(doc, ["global.liveData", "domains.vibrioAwareness"], now);
}
