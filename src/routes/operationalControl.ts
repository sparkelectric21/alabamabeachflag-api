...
import { AUDIT_PREFIX, SNAPSHOT_PREFIX, actorLabel, defaultOperationalControl, evaluateBeachEventsControl, evaluateFlagControl, evaluateVibrioAwarenessControl, parseOperationalControl, persistTransition, readOperationalControl } from "../operationalControl/store";
...
export async function handleOperationalControlGet(env: Env, now = new Date()): Promise<Response> {
	...
	return respond({ configuration: doc, effective: { gulfShoresFlags: evaluateFlagControl(doc, "gulfShoresFlags", now), orangeBeachFlags: evaluateFlagControl(doc, "orangeBeachFlags", now), beachEvents: evaluateBeachEventsControl(doc, now), vibrioAwareness: evaluateVibrioAwarenessControl(doc, now) } });
}
