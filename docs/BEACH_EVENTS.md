...
| Town of Dauphin Island | Official page directly embeds Events Calendar project `proj_Uup5Kfrh1ITrNrHIMFrvw`, backed by a public JSON endpoint | Town of Dauphin Island calendar publisher; Events Calendar infrastructure | None | Disabled; manual fallback only | The embed is directly configured on the official Town Wix site, but Events Calendar's Terms of Use prohibit automated/non-human access, systematic retrieval, and scrapers without written permission. Do not call the endpoint from production until written permission is obtained. |
