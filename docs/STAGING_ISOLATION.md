...
`src/local/eventStagingFixture.ts` is a local/miniflare-compatible runner, not an HTTP route. It accepts no URL or payload. Its committed calendar is parsed by the real iCalendar parser and passed through import, lifecycle, provider-health, snapshot, observation, audit, and duplicate-assessment paths. A scoped KV adapter exposes relative logical keys while every physical key retains `synthetic:events-isolation-v1:`. Cleanup enumerates and deletes only that exact physical prefix. The runner requires an empty synthetic namespace, consistent staging labels, and `STAGING_SYNTHETIC_FIXTURES_ENABLED=true`; the deployable default is false. Fixture audit keys use a per-run deterministic SHA-256-derived generator with a deterministic occurrence index, while production audit IDs retain their random default. Additional complete/304/validator scenarios remain covered by the ingestion regression suite rather than simulated network calls here.
...
Before a separately authorized isolated deployment: validate the staging hostname and Access audiences, confirm KV/D1 IDs differ from production without publishing them, prove Durable Object namespace separation, confirm no email binding and no dashboard cron, and leave all three staging flags false. The staging Worker hostname must be protected by staging-specific Access, and version preview URLs remain disabled in configuration so deployment cannot reopen a public preview bypass. Roll back by selecting the prior Worker version and rechecking bindings/triggers; never use a production binding as a fallback. Provider refresh, fixture execution, migrations, schedule creation, email configuration, Access changes, deployment, and external verification all require separate authorization.
