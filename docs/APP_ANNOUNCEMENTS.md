...
`GET /v1/app-announcement` is public and read-only. It returns `{ "status": "ok", "announcement": null }` when no announcement is currently active. An active announcement contains `id`, server-generated `revision`, `title`, `message`, `severity`, `startsAt`, `expiresAt`, nullable `actionTitle`/`actionUrl` fields, and an explicit audience represented by `scope` (`all` or `beaches`) plus `beachIds`. Supported targeting IDs are `gulf-shores`, `orange-beach`, `fort-morgan`, and `dauphin-island`. Severity is one of `information`, `notice`, `important`, or `critical`. Legacy records without audience fields are returned as `scope: "all"` with an empty `beachIds` array.
...
Input is strict plain text: ID 1–128 characters, title 1–80, message 1–500, action title 1–40, UTC ISO-8601 start/expiration timestamps, and expiration later than start. Action links use a code-owned allowlist: `alabamabeachflag.com` and `www.alabamabeachflag.com` allow normal paths, while `www.redcross.org` is limited to `/take-a-class/resources/learn-first-aid/`. Links must use HTTPS with an exact approved hostname and may not contain credentials, explicit ports, or fragments. Unexpected fields, markup/control characters, unapproved destinations, and wording that presents the app notice as Apple, NWS/NOAA, police, government, or emergency services are rejected. Factual operational wording such as “NWS data is temporarily unavailable” remains allowed.
...
The iOS app refreshes announcements on launch, pull-to-refresh, normal beach refresh, beach selection changes, and foreground return after five minutes. It caches a last successful response locally, but shows a cached item only inside its valid time window and when its audience includes the selected beach. Missing legacy scope means All Beaches. Failures are silent and cannot block beach data. Information, notice, and important notices are dismissible locally by revision; critical notices remain visible while active, and revised notices reappear. A future push feature can reuse the stable ID/revision contract, but no push fields or behavior exist today.
