...
`VERIFICATION_ALERTS_ENABLED` is the delivery-only kill switch. It is enabled in production after controlled staging validation and remains false in staging by default. The delivery interface uses the native `VERIFICATION_ALERT_EMAIL` binding, restricted in Wrangler to sender `alerts@alabamabeachflag.com` and fixed destination `operations@alabamabeachflag.com`. Plain-text messages contain only environment, alert type, slot, Central timestamp, overall status, affected names, and concise diagnostics. This avoids third-party credentials and infrastructure; Cloudflare sending logs/metrics provide delivery operations.
